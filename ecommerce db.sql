...
/* Q2 - inserting data into tables */

/* ques 2) INSERT INTO TABLE (can provide the below data to learners)*/
...
Select count(ID) as CustomerCount, Gender 
...
where ORD_AMOUNT >=3000) as Subquery 
group by Gender;
...
select supplier.* 
from supplier where supplier.supp_id 
in 
...
select catg.CAT_ID as Category_ID, catg.CAT_NAME as Category_Name, min(SP.SUPP_PRICE) as Product_Price from category as catg 
...
select prod.PRO_ID, prod.PRO_NAME, o.ORD_DATE 
from product prod
inner join supplier_pricing SP on SP.PRO_ID = prod.PRO_ID
...
from customer 
where customer.cus_name like 'A%' or 
